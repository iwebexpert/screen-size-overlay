...
**Screen Size Overlay** is a lightweight React component that displays the `current screen size` (width and height), shows distances to the previous and next breakpoints. Supports presets (`Tailwind CSS`, `Bootstrap`, `Foundation`, `Bulma`, `MUI`) or fully custom configurations.
...
- **Lightweight**: The library is less than **4KB** (gzip). No dependencies.
...
- **New in 1.5.0**:
  - **Modes**: `'visible'`, `'auto-hide'`, and `'auto-compact'` for controlling when the overlay is shown or hidden.
  - **`displayDuration`**: Configurable time (in ms) determining how long the overlay remains fully visible when in `auto-hide` or `auto-compact` mode.
...
| `mode`               | `'visible' \| 'auto-hide' \| 'auto-compact'`                                                                                        | `'visible'`      | **Determines when the overlay is shown or hidden**. <br/>• **`visible`**: always displayed <br/>• **`auto-hide`**: appears on resize, hides after `displayDuration` <br/>• **`auto-compact`**: shows only the breakpoint label unless hovered/resized |
| `displayDuration`    | `number`                                                                                                                            | `2000`           | **Time (in ms) the overlay remains fully visible** in `auto-hide` or `auto-compact` mode before it fades out or returns to compact view.                                                                                                              |
...
<ScreenSizeOverlay
  ...
  mode="auto-compact"
  // New in 1.5.0: Time (in ms) the overlay remains visible in 'auto-hide' and 'auto-compact' modes. Default: 2000 ms
